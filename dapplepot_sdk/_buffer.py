...
class EventBuffer:
    ...
    def push(self, event: dict) -> None:
        ...
        elif sid and 'sequence_index' in event:
            # Keep counter ahead of LangChain's pre-set seq so events pushed
            # without a seq (session_error, etc.) get a number that doesn't collide.
            seen = event['sequence_index']
            current = self._session_seqs.get(sid, 0)
            if seen >= current:
                self._session_seqs[sid] = seen + 1
